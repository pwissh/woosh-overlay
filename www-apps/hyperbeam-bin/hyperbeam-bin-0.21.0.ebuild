...
src_install() {
	...
	# Install top-level desktop file
	if [[ -f squashfs-root/hyperbeam.desktop ]]; then
		# Patch Exec line to point to /usr/bin/hyperbeam
		sed 's|Exec=AppRun|Exec=/usr/bin/hyperbeam|' squashfs-root/hyperbeam.desktop > "${T}/hyperbeam.desktop"

		insinto /usr/share/applications
		doins "${T}/hyperbeam.desktop"
	fi
